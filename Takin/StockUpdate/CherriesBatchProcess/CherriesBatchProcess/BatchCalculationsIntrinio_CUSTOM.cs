...
{
    public class BatchCalculationsIntrinio_CUSTOM
    {
        public static void Execute(Dictionary<string, string> config, int[] exchangesIds)
        {
            string connectionString = config[Constants.DB_ConnectionString_Quandl];
            int chooseSecuritiesForexchange = Int32.Parse(config[Constants.ChooseSecuritiesForExchange]);
            //DateTime startDate = DateTime.Today.AddDays(-1).AddYears(-6);
            DateTime startDateMissingPrc = DateTime.Today.AddDays(-30);
            DateTime endDate = DateTime.Today.AddDays(-1); ;

            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                conn.Open();
                Logger logger = new Logger(conn, Constants.IntrinioBatchCalculation);
                ...
                //    logger.WriteLog(Constants.Success, null, DateTime.MinValue, sp, "Succesfull shekel exchange rate filling", 0, null);
                //}
                //catch (Exception ex)
                //{
                //    shekelLeShekelSuccess = false;
                //    logger.WriteLog(Constants.Failure, null, DateTime.MinValue, sp, "Unsuccesfull shekel exchange rate filling", 0, ex);
                //}
                ...
                for (int i = 0; i < exchangesIds.Length; i++)
                {
                    ...
                    try
                    {
                        ...
                        command.Parameters.Add("@exchanges_list", SqlDbType.VarChar).Value = exchangesIds[i];   // exchangesString;
                        command.ExecuteNonQuery();
                        completeMissingPricesFillingSuccess = true;
                        logger.WriteLog(Constants.Success, null, DateTime.MinValue, sp, "Successfull CompleteMissingPrices filling. Exchange id " + exchangesIds[i], 0, null);
                    }
                    catch (Exception ex)
                    {
                        completeMissingPricesFillingSuccess = false;
                        logger.WriteLog(Constants.Failure, null, DateTime.MinValue, sp, "Unsuccessfull CompleteMissingPrices filling. Exchange id " + exchangesIds[i], 0, ex);
                    }
                }
                //try
                //{
                ...
                //    command.CommandText = sp;
                //    command.CommandTimeout = Constants.SPsTimeout;
                //    command.Parameters.Add("@dates_seq", SqlDbType.VarChar).Value = "range";     // !!!WARNING: ONCE HAVE TO RUN WITH 'ALL'
                //    command.Parameters.Add("@start_date", SqlDbType.DateTime).Value = startDateMissingPrc;  // endDate.AddDays(-4); ;
                //    command.Parameters.Add("@end_date", SqlDbType.DateTime).Value = endDate;
                ...
                //    logger.WriteLog(Constants.Success, null, DateTime.MinValue, sp, "Successfull CompleteMissingIndexPrices filling", 0, null);
                //}
                //catch (Exception ex)
                //{
                //    completeMissingIndexPricesFillingSuccess = false;
                //    logger.WriteLog(Constants.Failure, null, DateTime.MinValue, sp, "Unsuccessfull CompleteMissingIndexPrices filling", 0, ex);
                //}
                ...
                if (shekelLeShekelSuccess && setIsTradedSuccess && completeMissingPricesFillingSuccess && duplicateLastDayPricesSuccess)
                {
                    // Securities
                    ...
                    for (int i = 0; i < exchangesIds.Length; i++)
                    {
                        try
                        {
                            ...
                            command.Parameters.Add("@exchanges_list", SqlDbType.VarChar).Value = exchangesIds[i];
                            command.Parameters.Add("@is_index", SqlDbType.Int).Value = 0;   // from tbl_Securities
                            command.ExecuteNonQuery();
                            calcPriceReturnsSuccess = true;
                            logger.WriteLog(Constants.Success, null, DateTime.MinValue, sp, "Successfull securities price returns calculation. Exchange id " + exchangesIds[i], 0, null);
                        }
                        catch (Exception ex)
                        {
                            calcPriceReturnsSuccess = false;
                            logger.WriteLog(Constants.Failure, null, DateTime.MinValue, sp, "Unsuccessfull securities price returns calculation. Exchange id " + exchangesIds[i], 0, ex);
                        }
                    }
                    ...
                    //    command.ExecuteNonQuery();
                    //    //calcPriceReturnsSuccess = true;
                    //    logger.WriteLog(Constants.Success, null, DateTime.MinValue, sp, "Successfull indices price returns calculation.", 0, null);
                    //}
                    //catch (Exception ex)
                    //{
                    //    //calcPriceReturnsSuccess = false;
                    //    logger.WriteLog(Constants.Failure, null, DateTime.MinValue, sp, "Unsuccessfull indices price returns calculation.", 0, ex);
                    //}

                    //////======================================================================================
                    ////// TODO: DELETE LATER - CHANGED OLD SP TO ACCOMODATE THE NEW LOGIC
