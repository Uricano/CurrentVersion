...
namespace Cherries.TFI.BusinessLogic.GMath
{
    public static class cMath
    {
        ...
        public static double[] getSharpeRatioArr(List<double> arrRisk, List<double> arrRate)
        { // returns array of sharpe ratio values
            if ((arrRate.Count == 0) || (arrRisk.Count == 0)) return null;
            if (arrRate.Count != arrRisk.Count) return null;

            double[] arrSharpe = new double[arrRisk.Count];
            for (int iVals = 0; iVals < arrSharpe.Length; iVals++)
                if (arrRisk[iVals] != 0D) arrSharpe[iVals] = (arrRate[iVals] - cRiskFreeRates.YearlyRate) / arrRisk[iVals]; // Sharpe ratio calculation
                //if (arrRisk[arrRisk.Count - iVals - 1] != 0D) arrSharpe[iVals] = (arrRate[iVals] - cRiskFreeRates.YearlyRate) / arrRisk[arrRisk.Count - iVals - 1]; // Sharpe ratio calculation
                else arrSharpe[iVals] = 0D;
            return arrSharpe;
        }//getSharpeRatioArr
